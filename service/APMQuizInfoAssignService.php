...
class APMQuizInfoAssignService extends BaseService{
	...
	public function deleteQuizOnTest($exam_id) {
		// データベース接続
		$dao = new  T_YNS_Exam_QuizDao ();
		return $dao->deleteQuizOnTest ( $exam_id );
	}

	public function getRegisteredQuizList($exam_id) {
		// データベース接続
		$dao = new  T_YNS_Exam_QuizDao ();
		return $dao->getRegisteredQuizList($exam_id);
	}

	public function addQuizDataOnTest($dto) {
		// データベース接続
		$dao = new  T_YNS_Exam_QuizDao ( $this->pdo );
